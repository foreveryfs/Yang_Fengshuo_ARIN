...
def naiveSoftmaxLossAndGradient(
    ...
    gradCenterVec = y_hat@outsideVectors
...
def negSamplingLossAndGradient(
    ...
    z=-outsideVectors[indices]@centerWordVec
    z[0]*=-1
    y_hat=sigmoid(z)
    loss=-np.sum(np.log(y_hat))
    y_hat-=1
    y_hat[1:]*=-1
    gradCenterVec=y_hat@outsideVectors[indices]
    gradOutsideVecs=np.zeros_like(outsideVectors)
    grads=np.outer(y_hat,centerWordVec)
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        (tmpLoss,tmpgradCenterVecs,tmpgradOutsideVectors)=word2vecLossAndGradient(currentCenterWordVec,
                                                                                word2Ind[outsideWord],
                                                                                outsideVectors,
                                                                                dataset)
        loss+=tmpLoss
        gradCenterVecs[currentCenterWordIdx]+=tmpgradCenterVecs
