...
from models.module_ct import MLP, Decoder, Supp_Decoder
import numpy as np
import random
import cv2
...
class FewShotSeg(nn.Module):
    def __init__(self, pretrained_weights="deeplabv3"):
        ...
        self.fg_num = 100
        self.bg_num = 600
        self.mlp1 = MLP(257, self.fg_num)
        self.mlp2 = MLP(257, self.bg_num)
    ...
    def get_fg_pts(self, features, mask, pred_mask):
        ...
        #----------修改区-------------------------------------------------------------------------
        # 1. 检查维度并升维：F.interpolate 严格要求 4D 张量 (N, C, H, W)
        # 如果 fg_fts 是 (512, 256, 256)，unsqueeze(0) 后变成 (1, 512, 256, 256)
        # if fg_fts.dim() == 3:
        #     fg_fts = fg_fts.unsqueeze(0)
            
        # 2. 强行对齐空间维度到 257x257
        # fg_fts = F.interpolate(fg_fts, size=(257, 257), mode='bilinear', align_corners=True)
        
        # 3. 还原维度并展平：view 需要 512 作为第一维
        # squeeze(0) 将其变回 (512, 257, 257)
        # fg_fts = fg_fts.squeeze(0)
        #--------------------------------------------------------------------------------
        fg_prototypes = self.mlp1(fg_fts.view(512, 257 * 257)).permute(1, 0)
    ...
    def get_bg_pts(self, features, mask, pred_mask):
        ...
        # 如果 fg_fts 是 (512, 256, 256)，unsqueeze(0) 后变成 (1, 512, 256, 256)
        # if bg_fts.dim() == 3:
        #     bg_fts = bg_fts.unsqueeze(0)
            
        # 2. 强行对齐空间维度到 257x257
        # bg_fts = F.interpolate(bg_fts, size=(257, 257), mode='bilinear', align_corners=True)
        
        # 3. 还原维度并展平：view 需要 512 作为第一维
        # squeeze(0) 将其变回 (512, 257, 257)
        # bg_fts = bg_fts.squeeze(0)
        #--------------------------------------------------------------------------------
        bg_prototypes = self.mlp2(bg_fts.view(512, 257 * 257)).permute(1, 0)
