...
class TestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        next_idx = min(len(self.image_dirs) - 1, idx + 1)

        paths = [self.image_dirs[prev_idx], img_path, self.image_dirs[next_idx]]
        imgs = []
        for p in paths:
            _img = sitk.GetArrayFromImage(sitk.ReadImage(p))
            # normalized
            _img = (_img - _img.mean()) / (_img.std() + 1e-5) # 加个极小值防止除以0
            imgs.append(_img) # 此时 _img 形状通常是 (Depth, H, W)，但在 FSMIS 中通常 Depth=1

        # # 2. 将三个切片沿通道维度拼接 (通道0:前一帧, 通道1:当前帧, 通道2:后一帧)
        img_25d = np.concatenate(imgs, axis=1)
        # #-----------------------------------------------------------
        ...
        # Evaluation protocol.
        valid_mask = lbl.sum(axis=(1, 2)) > 0
        sample = {'id': img_path}

        #sample['image'] = torch.from_numpy(img[valid_mask]).float()
        sample['image'] = torch.from_numpy(img_25d[valid_mask]).float()
        
        sample['label'] = torch.from_numpy(lbl[valid_mask]).long()
